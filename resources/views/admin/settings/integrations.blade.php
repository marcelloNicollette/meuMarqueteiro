    ...
    <div style="padding:2rem;max-width:900px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.5rem">
            <div>
                ...
                <p style="font-size:.85rem;color:#6b7280;margin-top:.3rem">Fontes públicas de dados municipais integradas ao
                    assistente</p>
            </div>
            <a href="{{ route('admin.settings.index') }}" style="font-size:.85rem;color:#6b7280;text-decoration:none">←
                Configurações</a>
        </div>
        ...
        <form method="POST" action="{{ route('admin.settings.integrations.save') }}">
            ...
            @foreach ($grupos as $grupo => $apis)
                <div style="background:#fff;border-radius:12px;border:1px solid #e5e7eb;margin-bottom:1rem;overflow:hidden">
                    <div
                        style="padding:1rem 1.5rem;background:#f9fafb;border-bottom:1px solid #e5e7eb;display:flex;align-items:center;gap:.75rem">
                        <span
                            style="font-size:1.1rem">{{ $grupo === 'fiscal' ? '💰' : ($grupo === 'saude' ? '🏥' : ($grupo === 'educacao' ? '📚' : ($grupo === 'infraestrutura' ? '🏗️' : ($grupo === 'captação' ? '🎯' : '📊')))) }}</span>
                        <div>
                    ...
                    <div style="padding:.5rem 0">
                        @foreach ($apis as $key => $api)
                            <div
                                style="padding:.9rem 1.5rem;border-bottom:1px solid #f9fafb;display:flex;align-items:flex-start;gap:1rem">
                                ...
                                <div style="flex:1;min-width:0">
                                    <div style="display:flex;align-items:center;gap:.75rem;margin-bottom:.2rem">
                                        <span
                                            style="font-weight:600;font-size:.9rem;color:#0f1117">{{ $api['nome'] }}</span>
                                        @if ($api['gratuita'])
                                            <span
                                                style="padding:.1rem .5rem;background:#d1fae5;color:#065f46;border-radius:99px;font-size:.7rem;font-weight:600">GRATUITA</span>
                                        @else
                                            <span
                                                style="padding:.1rem .5rem;background:#fef3c7;color:#92400e;border-radius:99px;font-size:.7rem;font-weight:600">REQUER
                                                CADASTRO</span>
                                        @endif
                                        @if ($api['ativo'])
                                            <span
                                                style="padding:.1rem .5rem;background:#d1fae5;color:#065f46;border-radius:99px;font-size:.7rem;font-weight:600">✓
                                                ATIVA</span>
                                        @endif
                                    </div>
                                    <div style="font-size:.82rem;color:#6b7280;margin-bottom:.4rem">{{ $api['descrição'] }}
                                    </div>
                                    <div style="font-size:.75rem;color:#9ca3af">
                                        <a href="{{ $api['url'] }}" target="_blank"
                                            style="color:var(--gold);text-decoration:none">{{ $api['url'] }}</a>
                                    </div>
