...
class KnowledgeBaseController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Dados para a aba de URLs
        $urlQuery = MonitoredUrl::with('municipality')->orderByDesc('created_at');
        if (request('municipality_id')) {
            $urlQuery->where('municipality_id', request('municipality_id'));
        }
        $urls = $urlQuery->get();
        $urlStats = [
            'total'    => MonitoredUrl::count(),
            'indexadas' => MonitoredUrl::where('fetch_status', 'indexed')->count(),
            'falhas'   => MonitoredUrl::where('fetch_status', 'failed')->count(),
            'pendentes' => MonitoredUrl::whereIn('fetch_status', ['pending', 'fetching'])->count(),
            'chunks'   => MonitoredUrl::sum('chunks_count'),
        ];
        $municipalities = \App\Models\Municipality::where('subscription_active', true)->orderBy('name')->get();

        return view('admin.knowledge-base.index', compact('documents', 'categories', 'stats', 'urls', 'urlStats', 'municipalities'));
    }
